/**
 * Browser-safe Supabase client for the AI News Dashboard.
 *
 * Uses the anon key only. The service_role key must NEVER ship to the
 * browser. Reads are gated by RLS policies on the `articles` table.
 *
 * Set VITE_USE_MOCK=1 in .env to serve a static fixture for local
 * development without hitting Supabase.
 */
...
if (!isMockMode && (!url || !anon)) {
  ...
  console.warn(
    "[supabase] VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY missing. " +
      "Copy .env.example to .env and fill in the values, or set VITE_USE_MOCK=1."
  );
}
  ...
  if (!url || !anon) {
    throw new Error(
      "Supabase env vars missing. Set VITE_SUPABASE_URL and " +
        "VITE_SUPABASE_ANON_KEY in .env (or enable VITE_USE_MOCK=1)."
    );
  }
 ...
 * shape. Pass VITE_USE_MOCK=1 to serve a static fixture instead.
 */
